...
import (
	...
	"github.com/AstraProtocol/astra/v2/tests/e2e/util"
)
...
func (v *internalValidator) createKeyFromMnemonic(name, mnemonic string) error {
	kb, err := keyring.New(sdk.KeyringServiceName(), keyring.BackendTest, v.configDir(), nil, astrakr.Option())
	if err != nil {
		return err
	}
...
func (v *internalValidator) init() error {
	...
	appState, err := json.MarshalIndent(astraApp.ModuleBasics.DefaultGenesis(util.Cdc), "", " ")
	if err != nil {
